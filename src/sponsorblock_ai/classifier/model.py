...
def get_all_videos_from_pl_or_c_id(channel_id):
    continuation = None
    ...
    while True:
        if continuation is None:
            ...
            response = requests.get("https://www.youtube.com/playlist", params=params)
...
class ModelArguments:
    ...
    cache_dir: Optional[str] = field(
        default="models",
        metadata={
            "help": "Where to store the pretrained "
...
class InferenceArguments(ModelArguments):
    ...
    model_name_or_path: str = field(
        ...
        metadata={"help": "Path to pretrained model used for prediction"},
    )
    classifier_model_name_or_path: str = field(
        default="EColi/SB_Classifier", metadata={"help": "Use a pretrained classifier"}
    )

    max_videos: Optional[int] = field(
        default=None, metadata={"help": "The number of videos to test on"}
    )
    start_index: int = field(
        default=None, metadata={"help": "Video to start the evaluation at."}
    )
    channel_id: Optional[str] = field(
        default=None, metadata={"help": "Used to evaluate a channel"}
    )
    video_ids: str = field(default_factory=lambda: [], metadata={"nargs": "+"})

    output_as_json: bool = field(
        default=False, metadata={"help": "Output evaluations as JSON"}
    )

    min_probability: float = field(
        default=0.5,
        metadata={
            "help": "Remove all predictions whose classification "
    ...
    def __post_init__(self):
        ...
        if self.channel_id is not None:
            ...
            channel_video_ids = list(
                itertools.islice(
                    get_all_videos_from_pl_or_c_id(self.channel_id), start, end
                )
            )
            logger.info(f"Found {len(channel_video_ids)} for channel {self.channel_id}")

            self.video_ids += channel_video_ids


def get_model_tokenizer_classifier(
    inference_args: InferenceArguments, general_args: GeneralArguments
):
...
def get_model_tokenizer(
    ...
    general_args: Union[GeneralArguments, TrainingArguments] = None,
    config_args=None,
    model_type="seq2seq",
):
