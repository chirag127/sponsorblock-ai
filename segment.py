...
class SegmentationArguments:
    pause_threshold: int = field(
        default=2.5,
        metadata={
            "help": "When the time between words is greater than pause threshold, "
            + "force into a new segment"
        },
    )
...
def generate_labelled_segments(words, tokenizer, segmentation_args, sponsor_segments):
    ...
    return list(map(lambda x: add_labels_to_words(x, sponsor_segments), segments))
...
def generate_segments(words, tokenizer, segmentation_args):
    ...
    for index, (word, num_tokens) in enumerate(zip(words, num_tokens_list)):
        ...
        if (
            index == 0
            or word_start(words[index]) - word_end(words[index - 1])
            >= segmentation_args.pause_threshold
        ):
...
def extract_segment(words, start, end, map_function=None):
    ...
    return [map_function(words[i]) if to_transform else words[i] for i in range(a, b)]
