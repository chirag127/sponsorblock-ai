...
SEGMENT_MATCH_RE = rf"{_SEGMENT_START}\s*(?P<text>.*?)\s*(?:{_SEGMENT_END}|$)"
...
class DatasetArguments:
    ...
    processed_file: Optional[str] = field(
        ...
        metadata={"help": "Processed data file"},
    )
    processed_database: Optional[str] = field(
        default="processed_database.json",
        metadata={"help": "Processed database file"},
    )
    ...
    def __post_init__(self):
        ...
        if train_extension not in [
            "csv",
            "json",
        ]:
            raise AssertionError("`train_file` should be a csv or a json file.")
...
def load_datasets(dataset_args: DatasetArguments):
    ...
    if dataset_args.test_file is not None:
        data_files["test"] = os.path.join(dataset_args.data_dir, dataset_args.test_file)

    return load_dataset(
        "json", data_files=data_files, cache_dir=dataset_args.dataset_cache_dir
    )
...
class AdditionalTrainingArguments:
    ...
    save_steps: int = field(
        default=5000, metadata={"help": "Save checkpoint every X updates steps."}
    )
    eval_steps: int = field(
        default=25000, metadata={"help": "Run an evaluation every X steps."}
    )
    logging_steps: int = field(
        default=5000, metadata={"help": "Log every X updates steps."}
    )
...
def prepare_datasets(
    ...
    train_dataset = train_data_from_raw_datasets_and_args(raw_datasets, training_args)
...
def train_data_from_raw_datasets_and_args(raw_datasets, training_args):
