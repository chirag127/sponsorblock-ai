...
def _windows_write_string(s, out, skip_errors=True):
    ...
    GetStdHandle = ctypes.WINFUNCTYPE(ctypes.wintypes.HANDLE, ctypes.wintypes.DWORD)(
...
def safe_print(*objects, sep=" ", end="\n", out=None, encoding=None, flush=False):
    ...
    """
    output_string = sep.join(map(str, objects)) + end
    ...
    if (
        ...
        and _windows_write_string(output_string, out)
    ):
        return
