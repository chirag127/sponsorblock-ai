...
class PredictArguments(InferenceArguments):
    video_id: str = field(
        default=None, metadata={"help": "Video to predict segments for"}
    )
...
def filter_and_add_probabilities(predictions, classifier, min_probability):
    ...
    for prediction, probabilities in zip(predictions, classifications):
        ...
        if (prediction["category"] not in predicted_probabilities) or (
            classifier_category != "none" and classifier_probability > 0.5
        ):  # TODO make param
...
def predict(
    ...
    if words is None:
        words, _ = youtube_transcript_api_words.get_words(video_id, not_download=False)
        if not words:
...
def predict_sponsor_from_cleaned_texts(cleaned_texts, model, tokenizer):
    ...
    for cleaned_words in cleaned_texts:
        text = CustomTokens.EXTRACT_SEGMENTS_PREFIX.value + " ".join(cleaned_words)
        input_ids = tokenizer(text, return_tensors="pt", truncation=True).input_ids.to(
            model_device
        )
        ...
        decoded_outputs.append(tokenizer.decode(outputs[0], skip_special_tokens=True))
...
def segments_to_predictions(segments, model, tokenizer):
    ...
    cleaned_texts = [
        [x["cleaned"] for x in cleaned_segment] for cleaned_segment in segments
    ]
    ...
    for segment_matches, cleaned_batch, segment in zip(
        matches, cleaned_texts, segments
    ):
        ...
        for (
            ...
            segment_matches
        ):  # one segment might contain multiple sponsors/ir/selfpromos

            matched_text = match["text"].split()

            i1, j1, k1 = greedy_match(cleaned_batch, matched_text[:MATCH_WINDOW])
            i2, j2, k2 = greedy_match(cleaned_batch, matched_text[-MATCH_WINDOW:])
    ...
    for range in predicted_time_ranges:
        ...
        if prev_prediction is not None and (
            start_time <= prev_prediction["end"] <= end_time
            or (  # Merge overlapping segments
                range["category"]
                == prev_prediction[
                    "category"
                ]  # Merge disconnected segments if same category and within threshold
            ...
            final_predicted_time_ranges.append(
                {"start": start_time, "end": end_time, "category": range["category"]}
            )
...
def main():
    ...
    hf_parser = HfArgumentParser(
        (PredictArguments, SegmentationArguments, GeneralArguments)
    )
    (
        predict_args,
        segmentation_args,
    ...
    for video_id in predict_args.video_ids:
        ...
        for index, prediction in enumerate(predictions, start=1):
            _extracted_from_main_45(index, prediction)
        print()


# TODO Rename this here and in `main`
def _extracted_from_main_45(index, prediction):
    print(f"Prediction #{index}:")
    print(
        'Text: "',
        " ".join([w["text"] for w in prediction["words"]]),
        '"',
        sep="",
    )
    print(
        "Time:",
        seconds_to_time(prediction["start"]),
        "\u2192",
