...
class EvaluationArguments(InferenceArguments):
    ...
    output_file: Optional[str] = field(
        default="metrics.csv", metadata={"help": "Save metrics to output file"}
    )

    skip_missing: bool = field(
        default=False,
        metadata={
            "help": "Whether to skip checking for missing segments."
            + " If False, predictions will be made."
        },
    )
    skip_incorrect: bool = field(
        default=False,
        metadata={
            "help": "Whether to skip checking for incorrect segments."
            + " If False, classifications will be made on existing segments."
        },
    )
...
def calculate_metrics(labelled_words, predictions):
    ...
    metrics = {
        ...
        "true_negative": 0,
        "video_duration": (
            word_end(labelled_words[-1]) - word_start(labelled_words[0])
        ),
    }
    ...
    for index, word in enumerate(labelled_words):
        ...
        predicted_sponsor = any(
            p["start"] <= word["start"] <= p["end"] for p in predictions
        )

        if predicted_sponsor:
            # total_positive_time += duration
            if word.get("category") is None:
                metrics["false_positive"] += duration
            else:  # Is actual sponsor
                metrics["true_positive"] += duration
        elif word.get("category") is not None:  # Is actual sponsor
    ...
    metrics["f-score"] = (
        (2 * (metrics["precision"] * metrics["recall"]) / s) if s > 0 else 0
    )
...
def main():
    ...
    hf_parser = HfArgumentParser(
        (EvaluationArguments, DatasetArguments, SegmentationArguments, GeneralArguments)
    )
    ...
    if not os.path.exists(final_path):
        logger.error(
            "ERROR: Processed database not found.\n"
            "Run `python src/preprocess.py --update_database "
            + f'--do_create` to generate "{final_path}".'
        )
        return
    ...
    try:
        with tqdm(video_ids) as progress:
            for video_index, video_id in enumerate(progress):
                ...
                current_metrics = {"video_id": video_id}
                metric_count += 1
                ...
                if not evaluation_args.skip_missing:  # Make predictions
                    ...
                    if sponsor_segments:
                        ...
                        all_metrics["total_prediction_accuracy"] += current_metrics[
                            "accuracy"
                        ]
                        all_metrics["total_prediction_precision"] += current_metrics[
                            "precision"
                        ]
                        all_metrics["total_prediction_recall"] += current_metrics[
                            "recall"
                        ]
                        all_metrics["total_prediction_fscore"] += current_metrics[
                            "f-score"
                        ]
                        ...
                        postfix_info.update(
                            {
                                "accuracy": all_metrics["total_prediction_accuracy"]
                                / metric_count,
                                "precision": all_metrics["total_prediction_precision"]
                                / metric_count,
                                "recall": all_metrics["total_prediction_recall"]
                                / metric_count,
                                "f-score": all_metrics["total_prediction_fscore"]
                                / metric_count,
                            }
                        )
                ...
                if not evaluation_args.skip_incorrect and sponsor_segments:
                    ...
                    for sponsor_segment in sponsor_segments:
                        ...
                        cleaned_texts.append(clean_text(sponsor_segment["text"]))
                    ...
                    if segments_to_check:  # Some segments to check
                        ...
                        current_metrics["num_segments"] = len(segments_to_check)
                    ...
                    if all_metrics["classifier_segment_count"] > 0:
                        postfix_info["classifier_accuracy"] = (
                            all_metrics["classifier_segment_correct"]
                ...
                if missed_segments or incorrect_segments:
                    ...
                    if evaluation_args.output_as_json:
                        ...
                        safe_print(f"Issues identified for {video_id} (#{video_index})")
                        # Potentially missed segments
                        # (model predicted, but not in database)
                        if missed_segments:
                            safe_print(" - Missed segments:")
                            segments_to_submit = []
                            for i, missed_segment in enumerate(
                                missed_segments, start=1
                            ):
                                safe_print(
                                    f"\t#{i}:",
                                    seconds_to_time(missed_segment["start"]),
                                    "-->",
                                    seconds_to_time(missed_segment["end"]),
                                )
                                safe_print(
                                    '\t\tText: "', missed_segment["text"], '"', sep=""
                                )
                                safe_print(
                                    "\t\tCategory:", missed_segment.get("category")
                                )
                            ...
                            safe_print(
                                f"\tSubmit: https://www.youtube.com/watch?v={video_id}"
                                + f"#segments={json_data}"
                            )

                        # Incorrect segments (in database, but incorrectly classified)
                        if incorrect_segments:
                            safe_print(" - Incorrect segments:")
                            for i, incorrect_segment in enumerate(
                                incorrect_segments, start=1
                            ):
                                safe_print(
                                    f"\t#{i}:",
                                    seconds_to_time(incorrect_segment["start"]),
                                    "-->",
                                    seconds_to_time(incorrect_segment["end"]),
                                )

                                safe_print(
                                    '\t\tText: "',
                                    incorrect_segment["text"],
                                    '"',
                                    sep="",
                                )
                                safe_print("\t\tUUID:", incorrect_segment["uuid"])
                                safe_print("\t\tVotes:", incorrect_segment["votes"])
                                safe_print("\t\tViews:", incorrect_segment["views"])
                                safe_print("\t\tLocked:", incorrect_segment["locked"])
